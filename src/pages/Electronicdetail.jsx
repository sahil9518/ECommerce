...
import { electroapi } from "../store/actions/electronicasync"
import { useDispatch, useSelector } from "react-redux"
import Loader from "../components/Loader"
import { removeproduct } from "../store/reducer/ElectronicSlice"
import Form from 'react-bootstrap/Form';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';


const Electronicdetail = ({cart,setcart}) => {

  const{id} =useParams()
  const {value} = useSelector((state) => state.electronic)
  console.log("val",value);
  const dispatch=useDispatch()


  useEffect(()=>{
    dispatch(electroapi(id));
    return ()=>{
